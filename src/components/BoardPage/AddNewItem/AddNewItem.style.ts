...
export const AddNewItemHolder = styled.div < AddNewItemType > `
  padding: ${(props) => (props.type === 'list' && props.isAddList ? '10px' : '0px')};
  background: ${(props) => (props.type === 'list' && props.isAddList
...
export const AddNewItemContainer = styled.div < AddNewItemType > `
  ...
  width: 272px;
  align-items: center;
  background: ${(props) => (props.type === 'list' ? props.theme.colors.gray : 'none')};
